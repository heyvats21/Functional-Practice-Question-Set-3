...
//   cars.find(({ make, model, year }) => make === "Toyota" && year > 2010);
...
//   students.filter(({ grade }) => grade > givenGrade);
...
//   games.filter(({ genre }) => genre === givenGenre);
...
//   numbers.reduce((acc, curr) => (curr % 2 === 0 ? acc + curr : acc));
...
//   products.reduce((acc, curr) => (acc = acc + curr.price * curr.quantity), 0);
...
//   numbers.reduce((acc, curr) => (curr < acc ? (acc = curr) : acc));
